...
void engine::adjustmentWindow(){
	...
	ImGui::SliderFloat2( "Position", (float*)&viewPosition, 0.0f, 1024.0f, "%.3f" );
	ImGui::SliderFloat( "Angle", &viewAngle, -3.14159265f, 3.14159265f, "%.3f" );
	ImGui::SliderFloat( "Max Distance", &maxDistance, 10.0f, 5000.0f, "%.3f" );
	ImGui::SliderInt( "Horizon", &horizonLine, 0, 3000, "%d" );
	ImGui::SliderFloat( "Height Scale", &heightScalar, 0.0f, 1500.0f, "%.3f" );
	ImGui::SliderFloat( "Side-to-Side Offset", &offsetScalar, 0.0f, 300.0f, "%.3f" );
	ImGui::SliderFloat( "Step Increment", &stepIncrement, 0.0f, 0.5f, "%.3f" );
	ImGui::SliderFloat( "FoV", &FoVScalar, 0.001f, 15.0f, "%.3f" );

	ImGui::SliderInt( "Linear Sampling", &linearTextures, 0, 1, "%d" );

	ImGui::Checkbox( "Height follows Player Height", &adaptiveHeight );
	ImGui::Text( "" );
	ImGui::SliderFloat( "View Bump", &viewBump, 0.0f, 500.0f, "%.3f" );
	ImGui::SliderFloat( "Minimap Scalar", &minimapScalar, 0.1f, 5.0f, "%.3f" );
	ImGui::Text( "" );
	ImGui::SliderFloat( "Fog Scale", &fogScalar, 0.0f, 1.5f, "%.3f" );
