...
void drawVerticalLine( const uint x, const float yBottom, const float yTop, const uvec4 col ){
  ...
  const int yMin = clamp( int( yBottom ), 0, imageSize( current ).y );
  const int yMax = clamp( int(  yTop  ), 0, imageSize( current ).y );
...
void main() {
  const float wPixels    = imageSize( current ).x;
  const float hPixels    = imageSize( current ).y;
