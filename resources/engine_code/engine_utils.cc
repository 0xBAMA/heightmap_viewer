...
void engine::createWindow() {
	...
	clearColor = ImVec4( 0.618f, 0.618f, 0.618f, 1.0f );
...
void engine::drawEverything() {
	...
	glGetQueryObjectui64v( queryID[0], GL_QUERY_RESULT, &startTime );
	glGetQueryObjectui64v( queryID[1], GL_QUERY_RESULT, &stopTime );
	secondPassFrameTimeMs = ( stopTime - startTime ) / 1000000.0f;
...
void engine::loadMap ( int index ) {
	...
	if( index == 31 ){
		...
		if( error )
			cout << "error loading heightmap data from " << heightmapPath << endl;

		error = lodepng::decode( colormap, cWidth, cHeight, colormapPath.c_str() );
		if( error )
			cout << "error loading colormap data from " << colormapPath << endl;
...
void engine::generateDiamondSquare(){
	...
	// cardinal directions
	glm::vec3 n = 0.15f * glm::normalize( glm::vec3( scale * ( myPoint - erosionModel[x + 1][y]), 1.0f, 0.0f )); // Positive X
	n += 0.15f * glm::normalize( glm::vec3( scale * ( erosionModel[x - 1][y] - myPoint ), 1.0f, 0.0f ) ); // Negative X
	n += 0.15f * glm::normalize( glm::vec3( 0.0f, 1.0f, scale * ( myPoint - erosionModel[x][y + 1] ) ) );  // Positive Y
	n += 0.15f * glm::normalize( glm::vec3( 0.0f, 1.0f, scale * ( erosionModel[x][y - 1] - myPoint ) ) );  // Negative Y
...
void engine::erode( int steps ){
	...
	for( int i = 0; i < steps; i++ ){
		...
		while ( p.volume > 0.0f ) {
			...
			p.volume *= ( 1.0f - dt * evaporationRate ); // evaporate some of the droplet
		}
	}
}
...
void engine::prepareMapsFromErosionModel(){
	...
	for(int x = 0; x < worldX; x++){
		for(int y = 0; y < worldY; y++){
			...
			colormap[ index + 0 ] = static_cast< unsigned char >( ( ( normalRef.x + 1.0f ) / 2.) * 255 );
			colormap[ index + 1 ] = static_cast< unsigned char >( ( ( normalRef.y + 1.0f ) / 2.) * 255 );
			colormap[ index + 2 ] = static_cast< unsigned char >( ( ( normalRef.z + 1.0f ) / 2.) * 255 );
